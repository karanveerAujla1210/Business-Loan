...
module.exports = {
	...
	createJob: async data => {
		...
		try {
			...
			//ToDo : Need to remove this code and place in user controler instead
	...
	getCurrentJob: async idUser => {
		try {
			...
			var job = await JobMaster.findOne({
				...
				where: {
					posterId: idUser,
					[Op.or]: [{ status: 'open' }, { status: 'new' },{status:'in-progress'}]
				}
			});
			if (job) {
				...
				job.dataValues.skills = jobSkills;
			}
